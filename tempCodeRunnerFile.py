...
def clean_text(text):
    ...
    text = re.sub(r',\s*$', '', text)  # Remove commas at the end of the sentence
    
    # Remove all commas
    text = re.sub(r',', '', text)  # Remove all commas globally
    
    # Remove lengths (numbers) at the end of the sentence
    text = re.sub(r'\d+\s*$', '', text)  # Remove trailing numbers with optional space
